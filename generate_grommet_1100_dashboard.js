/**
 * generate_grommet_1100_dashboard.js
 * The Grommet専用・No.1〜1100独立ダッシュボード生成（スタンドアロンHTML・XLSX不要）
 * node generate_grommet_1100_dashboard.js
 */

const fs = require('fs');
const GROMMET_1100 = require('./grommet_1100.js');

const OUTPUT = "C:\\Users\\hiro\\Desktop\\LEAGUE\\grommet_1100_dashboard.html";
...
console.log("grommet_1100.js 読み込み中...");

const products = [];
for (const row of GROMMET_1100) {
  const no       = row[0];
...
console.log(`The Grommet 1100 製品数: ${products.length} 件 / メールあり: ${emailCount} 件`);
...
<title>🟢 The Grommet No.1-1100 専用ダッシュボード</title>
<style>
...
<header>
  <div class="h-top">
    ...
    <h1>The Grommet No.1-1100 専用ダッシュボード — 独立採番・厳選アメリカ製品キュレーション</h1>
  </div>
  <div class="h-stats">
