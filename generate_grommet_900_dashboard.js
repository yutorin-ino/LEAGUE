/**
 * generate_grommet_900_dashboard.js
 * The Grommet専用・No.1〜900独立ダッシュボード生成（スタンドアロンHTML・XLSX不要）
 * node generate_grommet_900_dashboard.js
 */

const fs = require('fs');
const GROMMET_900 = require('./grommet_900.js');

const OUTPUT = "C:\\Users\\hiro\\Desktop\\LEAGUE\\grommet_900_dashboard.html";
...
console.log("grommet_900.js 読み込み中...");

const products = [];
for (const row of GROMMET_900) {
  const no       = row[0];
...
console.log(`The Grommet 900 製品数: ${products.length} 件 / メールあり: ${emailCount} 件`);
...
<title>🟢 The Grommet No.1-900 専用ダッシュボード</title>
<style>
...
<header>
  <div class="h-top">
    ...
    <h1>The Grommet No.1-900 専用ダッシュボード — 独立採番・厳選アメリカ製品キュレーション</h1>
  </div>
  <div class="h-stats">
