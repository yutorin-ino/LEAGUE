 ...
 * The Grommet 掲載製品限定ダッシュボード生成
 * node generate_grommet_dashboard.js
 */
...
const OUTPUT = "C:\\Users\\hiro\\Desktop\\LEAGUE\\grommet_dashboard.html";
...
for (let i = 1; i < rows.length; i++) {
  ...
  if (!ec_site.includes('The Grommet')) continue;  // The Grommet のみ
...
console.log(`The Grommet製品数: ${products.length} 件 / メールあり: ${emailCount} 件`);
...
<title>🟢 The Grommet 限定ダッシュボード</title>
<style>
...
<header>
  <div class="h-top">
    ...
    <h1>The Grommet 限定ダッシュボード — 革新的アメリカ製品キュレーション</h1>
  </div>
  <div class="h-stats">
    <span class="badge badge-total">総数 ${products.length} 件</span>
    <span class="badge badge-email">✉ メールあり ${emailCount} 件</span>
    <span class="badge" style="background:#fff;color:#1a6b2e;">🏆 厳選審査済み製品のみ</span>
  </div>
</header>
...
function renderTable(list) {
  ...
  tbody.innerHTML = list.map(p => {
    ...
    return \`<tr class="\${rowCls}" data-id="\${p.no}">
      <td><input type="checkbox" \${p.has_email ? '' : 'disabled'} \${selectedIds.has(p.no) ? 'checked' : ''} onchange="toggleRow(this, '\${p.no}')"></td>
